...
import ProductBox from "../inputs/ProductInput";
...
const MenuModal = ({
        ...
        <div className="w-full flex flex-col justify-center items-center h-full">
          <div className="w-2/3 ">
            <EmptyState
              src={svgScr}
              title="Oops!"
              text="Seems you don't have enough products to create a Menu! You need to products to create a Menu."
            />
          </div>
        </div>
      )}
    ...
    bodyContent = (
      <div className="flex flex-col gap-4">
        ...
        <div className="flex w-full flex-row items-center gap-4 pt-2 ">
          ...
          <Button className="w-full" type="submit" onClick={onNext}>
            Continue
          </Button>
        </div>
      </div>
  ...
  async function onSubmit(data: MenuFormValues) {
    ...
    const menu = await createMenu(data, id);
    if (menu.error) {
