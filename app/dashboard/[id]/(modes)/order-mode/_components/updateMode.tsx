...
import {
  Category, Item_Menu_Order, Item_Menu_Product_Order, Item_Product_Order, Menu, Menu_Product, Order, Product, Restaurant, User,
} from "@prisma/client";
import { LogOut, Plus, ScrollText, Search, X } from "lucide-react";
...
const orderFormSchema = z.object({
  ...
  discount: z
  .string().refine((val) => val === "" || /^(?:\d{1,2}(?:\.\d{1,2})?|\d{1,2})$/.test(val))
  .optional(),
  ...
  orderId: z.string({ required_error: "Provide orderId" }),
...
type OrderWithProductsAndMenus = Order & {
  Item_Product_Order: (Item_Product_Order & { product: Product | null })[];
  Item_Menu_Order: (Item_Menu_Order & {
    Item_Menu_Product_Order: (Item_Menu_Product_Order & {
...
export const OrderMode = ({
  ...
  orders: Order[];
  order: OrderWithProductsAndMenus;
  ...
  const form = useForm<OrderFormValues>({
    ...
    defaultValues: {
      ...
      observations: order.observation ? order.observation : undefined,
    },
  });
  ...
  useEffect(() => {
    const productArray = order.Item_Product_Order.map((item) => {
      if (item.product) {
        return {
          id: item.productId,
    ...
    const menuArray = order.Item_Menu_Order.map((item) => {
      const menuItems = item.Item_Menu_Product_Order.map((menuItem) => {
        if (menuItem.product) {
          return {
      ...
      if (menuItems.length > 0) {
        menuItem.subitems = menuItems;
      }

      return menuItem;
    });
    const allItems = [...productArray, ...menuArray];
  ...
  const calculateTotal = () => {
    ...
    let discount = 0;
    if (discountField && parseInt(discountField) < 100) {
      discount = (parseInt(discountField) * 0.01) * totalBeforeTax;
    }
    const total = totalBeforeTax - discount;
    const tax = itemsField.reduce((acc, item) => acc + item.tax, 0) / itemsField.length;
  ...
  async function onSubmit(data: OrderFormValues) {
    setIsLoading(true);
    const message = await updateOrder(data);
  ...
  async function onSubmitPay(data: OrderFormValues) {
    setIsLoading(true);

    const message = await payOrder(data);
    if (typeof message != 'string') {
      toast.error(message.error);
    } else {
  ...
  async function onSubmitCancel(data: OrderFormValues) {

    setIsLoading(true);
    const message = await updateStatus(data.orderId, "/dashborad", 'CANCELED');
    if (message.error) {
      toast.error(message.error);
    } else {
      toast.success('Order Canceled');
      router.push(`/dashboard/${params.id}/orders`)
    }
    setIsLoading(false);
  }
    ...
    <div className="flex h-screen flex-row ">
      <div className="flex w-5/6 flex-col px-1.5 lg:w-4/6 lg:items-center">
        <div className="flex flex-row w-full justify-center gap-1 p-6 xl:gap-3">
          ...
          <Button
            variant={"outline"}
            onClick={() => router.push(`/dashboard/${params.id}/order-mode`)}
            className="w-1/6 shadow-sm"
          >
        ...
        <div className="grid mt-3 grid-cols-2 gap-2 overflow-y-scroll pb-3 md:grid-cols-3 lg:grid-cols-4 xl:grid-cols-5">
              ...
              <div className="w-full flex flex-col justify-center items-center h-full">
                <div className="w-2/3 ">
                  <EmptyState
                    src={svgScr}
                    title="Oops!"
                    text="Seems the cart is empty! You can't update a order with the cart empty."
              ...
              <div className="bg-card border-t-1 flex flex-col p-4">
                ...
                <div className="flex space-x-4">
                  ...
                  <Dialog onOpenChange={handleOpenConfirmModal}>
                    <DialogTrigger asChild>
                      <Button
                        className="flex-grow bg-green-600 hover:bg-green-500"
                        variant="default"
                      >
                        Pay
                      </Button>
                  ...
                  <AlertDialog onOpenChange={handleOpenConfirmModal} >
                    <AlertDialogTrigger asChild>
                      <Button
                        className="w-1/6 bg-red-600 hover:bg-red-500"
                        variant="default"
                      >
                        Cancel
                      </Button>
                    </AlertDialogTrigger>
                    <AlertDialogContent className="max-h-[425px] overflow-auto bg-white sm:max-w-[630px] xl:max-h-[625px]">
                      <Form {...form}>
                        <form
                          onSubmit={form.handleSubmit(onSubmitCancel)}
                          className="mt-2.5 space-y-2"
                        >
                          <AlertDialogHeader>
                          ...
                          <AlertDialogFooter>
                            ...
                            <AlertDialogAction type='submit' className="bg-red-600 text-white hover:bg-red-700">Confirm</AlertDialogAction>
                          </AlertDialogFooter>
                        </form>
                      </Form>
                    </AlertDialogContent>
